...
export default class StripeV3PaymentStrategy implements PaymentStrategy {
    ...
    async initialize(options: PaymentInitializeOptions): Promise<InternalCheckoutSelectors> {
        ...
        const { initializationData: { stripePublishableKey, stripeConnectedAccount, cardFormIndividualElements } } = paymentMethod;

        this._individualPaymentComponents = cardFormIndividualElements;
    ...
    private _mountElement(methodId: string): Promise<StripeElement> {
        ...
        return new Promise((resolve, reject) => {
            ...
            switch (stripeElementType) {
                case StripeElementType.CreditCard:
                    if (this._individualPaymentComponents && isIndividualCardElementOptions(options)) {
